...
class NewEncoder(nn.Module):
    ...
    def forward(self, title_emb_input):
        word_att = self.droupt(self.attention(self.droupt(title_emb_input)))
        attention_weight = self.news_layer(word_att)
        attention_weight = torch.unsqueeze(attention_weight, dim=2)
        new_emb = torch.sum(word_att * attention_weight, dim=1)
        return new_emb
